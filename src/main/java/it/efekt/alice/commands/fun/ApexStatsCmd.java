...
public class ApexStatsCmd extends Command {
    ...
    public void onCommand(MessageReceivedEvent e) {
        ...
        if (getArgs().length == 1){
            ...
            if (getPlayerInfo(playerName) == null || getPlayerInfo(playerName).get("playerfound").getAsBoolean() == false){
                e.getChannel().sendMessage("Nie znaleziono takiego gracza").queue();
                return;
            }

            JsonObject playerInfo = getPlayerInfo(playerName);
            ...
            embedBuilder.setTitle("APEX - Statystyki (Eksperymentalne)");
    ...
    private JsonObject getPlayerInfo(String playerName){
    ...
    private String getPlayerId(String playerName){
        ...
        try {
            URL url = new URL("https://apextab.com/api/search.php?platform=pc&search=" + playerName);
            HttpURLConnection connection = (HttpURLConnection) url.openConnection();
            connection.setRequestProperty("User-Agent", "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.11 (KHTML, like Gecko) Chrome/23.0.1271.95 Safari/537.11");
